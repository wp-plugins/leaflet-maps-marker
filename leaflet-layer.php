...
if (!empty($action)) {
  ...
  elseif ($action == 'deleteboth') {
		...
		$wpdb->query( $result2 );
		$wpdb->query( "OPTIMIZE TABLE $table_name_layers" );
        echo '<p><div class="updated" style="padding:10px;">' . __('Layer and assigned markers have been successfully deleted','lmm') . '</div><a class=\'button-secondary\' href=\''.WP_ADMIN_URL.'admin.php?page=leafletmapsmarker_layers\'>' . __('show all layers','lmm') . '</a>&nbsp;&nbsp;&nbsp;<a class=\'button-secondary\' href=\''.WP_ADMIN_URL.'admin.php?page=leafletmapsmarker_layer\'>' . __('add new layer','lmm') . '</a><iframe src="http://www.mapsmarker.com/counter/go.php?id=layer_delete" frameborder="0" height="0" width="0" name="counter" scrolling="no"></iframe></p>';
  }
elseif ($action == 'delete') {
		...
		$wpdb->query( $result2 );
		$wpdb->query( "OPTIMIZE TABLE $table_name_layers" );
		wp_register_script( 'counter-layer-delete', 'http://www.mapsmarker.com/counter/go.php?id=layer_delete', array(), NULL, false);
		wp_enqueue_script( 'counter-layer-delete' );
		...
		<table class="widefat fixed">
			...
			<tr>
				...
				<td id="wmscheckboxes">
					...
					if ($panel == 1) {
						if ($mapwidthunit == 'px') {
							echo '<div id="panel" style="display:block; background: ' . addslashes($lmm_options[ 'defaults_layer_panel_background_color' ]) . '; width:' . ($mapwidth - 10).$mapwidthunit . '; padding:5px; border:1px solid #ccc;">'.PHP_EOL;
						}
						if ($mapwidthunit == '%') {
							echo '<div id="panel" style="display:block; background: ' . addslashes($lmm_options[ 'defaults_layer_panel_background_color' ]) . '; width:' . ($mapwidth - 2).$mapwidthunit . '; padding:5px; border:1px solid #ccc;">'.PHP_EOL;
						}
						echo '<span style="' . addslashes($lmm_options[ 'defaults_layer_panel_paneltext_css' ]) . ';">' . (($name == NULL) ? __('if set, layername will be inserted here','lmm') : $name) . '</span><span style="float:right;width:100px;text-align:right;">';
							if ( (isset($lmm_options[ 'defaults_layer_panel_kml' ] ) == TRUE ) && ( $lmm_options[ 'defaults_layer_panel_kml' ] == 1 ) ) {
							echo '<a href="' . LEAFLET_PLUGIN_URL . '/leaflet-kml.php?layer=' . $id . '" style="text-decoration:none;" title="' . __('Export as KML for Google Earth/Google Maps','lmm') . '" target="_blank"><img src="' . LEAFLET_PLUGIN_URL . '/img/icon-kml.png" width="14" height="14" alt="KML-Logo" style="background:no-repeat;margin:0px;padding:0px;" /></a>&nbsp;';
							}
							if ( (isset($lmm_options[ 'defaults_layer_panel_geojson' ] ) == TRUE ) && ( $lmm_options[ 'defaults_layer_panel_geojson' ] == 1 ) ) {
							echo '<a href="' . LEAFLET_PLUGIN_URL . '/leaflet-geojson.php?layer=' . $id . '&callback=jsonp" style="text-decoration:none;" title="' . __('Export as GeoJSON','lmm') . '" target="_blank"><img src="' . LEAFLET_PLUGIN_URL . '/img/icon-json.png" width="14" height="14" alt="GeoJSON-Logo" style="background:no-repeat;margin:0px;padding:0px;" /></a>&nbsp;';
							}
...
'<td>' . $row['mopenpopup'] . '</td>' : '';
	...
	$column_kml = ((isset($lmm_options[ 'misc_marker_listing_columns_kml' ] ) == TRUE ) && ( $lmm_options[ 'misc_marker_listing_columns_kml' ] == 1 )) ? '<td style="text-align:center;"><a href="' . LEAFLET_PLUGIN_URL . '/leaflet-kml.php?marker=' . $row['markerid'] . '" target="_blank">' . __('KML','lmm') . '</a></td>' : '';
	$column_geojson = ((isset($lmm_options[ 'misc_marker_listing_columns_geojson' ] ) == TRUE ) && ( $lmm_options[ 'misc_marker_listing_columns_geojson' ] == 1 )) ? '<td style="text-align:center;"><a href="' . LEAFLET_PLUGIN_URL . '/leaflet-geojson.php?marker=' . $row['markerid'] . '&callback=jsonp" target="_blank">' . __('GeoJSON','lmm') . '</a></td>' : '';
	$column_wikitude = ((isset($lmm_options[ 'misc_marker_listing_columns_wikitude' ] ) == TRUE ) && ( $lmm_options[ 'misc_marker_listing_columns_wikitude' ] == 1 )) ? '<td style="text-align:center;"><a href="' . LEAFLET_PLUGIN_URL . '/leaflet-wikitude.php?marker=' . $row['markerid'] . '" target="_blank">' . __('Wikitude','lmm') . '</a></td>' : '';
...
(function($) {
  selectlayer = new L.Map("selectlayer", { crs: <?php echo $lmm_options['misc_projections'] ?> });
	...
	{
	<?php if ( (isset($lmm_options[ 'controlbox_osm_mapnik' ]) == TRUE ) && ($lmm_options[ 'controlbox_osm_mapnik' ] == 1 ) )
			echo "'" . addslashes($lmm_options[ 'default_basemap_name_osm_mapnik' ]) . "': osm_mapnik,";
		if ( (isset($lmm_options[ 'controlbox_osm_osmarender' ]) == TRUE ) && ($lmm_options[ 'controlbox_osm_osmarender' ] == 1 ) )
			echo "'" . addslashes($lmm_options[ 'default_basemap_name_osm_osmarender' ]) . "': osm_osmarender,";
		if ( (isset($lmm_options[ 'controlbox_mapquest_osm' ]) == TRUE ) && ($lmm_options[ 'controlbox_mapquest_osm' ] == 1 ) )
			echo "'" . addslashes($lmm_options[ 'default_basemap_name_mapquest_osm' ]) . "': mapquest_osm,";
		if ( (isset($lmm_options[ 'controlbox_mapquest_aerial' ]) == TRUE ) && ($lmm_options[ 'controlbox_mapquest_aerial' ] == 1 ) )
			echo "'" . addslashes($lmm_options[ 'default_basemap_name_mapquest_aerial' ]) . "': mapquest_aerial,";
		if ( (isset($lmm_options[ 'controlbox_ogdwien_basemap' ]) == TRUE ) && ($lmm_options[ 'controlbox_ogdwien_basemap' ] == 1 ) )
			echo "'" . addslashes($lmm_options[ 'default_basemap_name_ogdwien_basemap' ]) . "': ogdwien_basemap,";
		if ( (isset($lmm_options[ 'controlbox_ogdwien_satellite' ]) == TRUE ) && ($lmm_options[ 'controlbox_ogdwien_satellite' ] == 1 ) )
			echo "'" . addslashes($lmm_options[ 'default_basemap_name_ogdwien_satellite' ]) . "': ogdwien_satellite,";
		if ( (isset($lmm_options[ 'controlbox_custom_basemap' ]) == TRUE ) && ($lmm_options[ 'controlbox_custom_basemap' ] == 1 ) )
			echo "'".addslashes($lmm_options[ 'custom_basemap_name' ])."': custom_basemap,";
		if ( (isset($lmm_options[ 'controlbox_custom_basemap2' ]) == TRUE ) && ($lmm_options[ 'controlbox_custom_basemap2' ] == 1 ) )
			echo "'".addslashes($lmm_options[ 'custom_basemap2_name' ])."': custom_basemap2,";
		if ( (isset($lmm_options[ 'controlbox_custom_basemap3' ]) == TRUE ) && ($lmm_options[ 'controlbox_custom_basemap3' ] == 1 ) )
			echo "'".addslashes($lmm_options[ 'custom_basemap3_name' ])."': custom_basemap3";
	?>
	},
	...
	<?php 
		if ( (isset($lmm_options[ 'overlays_custom' ] ) == TRUE ) && ( $lmm_options[ 'overlays_custom' ] == 1 ) )
			echo "'".addslashes($lmm_options[ 'overlays_custom_name' ])."': overlays_custom,";
		if ( (isset($lmm_options[ 'overlays_custom2' ] ) == TRUE ) && ( $lmm_options[ 'overlays_custom2' ] == 1 ) )
			echo "'".addslashes($lmm_options[ 'overlays_custom2_name' ])."': overlays_custom2,";
		if ( (isset($lmm_options[ 'overlays_custom3' ] ) == TRUE ) && ( $lmm_options[ 'overlays_custom3' ] == 1 ) )
			echo "'".addslashes($lmm_options[ 'overlays_custom3_name' ])."': overlays_custom3,";
		if ( (isset($lmm_options[ 'overlays_custom4' ] ) == TRUE ) && ( $lmm_options[ 'overlays_custom4' ] == 1 ) )
			echo "'".addslashes($lmm_options[ 'overlays_custom4_name' ])."': overlays_custom4";
	?>
	},
