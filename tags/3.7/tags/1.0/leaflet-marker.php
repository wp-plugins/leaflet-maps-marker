...
if (! wp_verify_nonce($markernonce, 'marker-nonce') ) die('<br/>'.__('Security check failed - please call this function from the according Leaflet Maps Marker admin page!','lmm').'');
  ...
  elseif ($action == 'edit') {
	  if ( ($lat_check != NULL) && ($lon_check != NULL) ) {
		...
		$result = $wpdb->prepare( "UPDATE $table_name_markers SET markername = %s, basemap = %s, layer = %d, lat = %s, lon = %s, icon = %s, popuptext = %s, zoom = %d, openpopup = %d, mapwidth = %d, mapwidthunit = %s, mapheight = %d, panel = %d, updatedby = %s, updatedon = %s, controlbox = %d, overlays_custom = %s, overlays_custom2 = %s, overlays_custom3 = %s, overlays_custom4 = %s, wms = %d, wms2 = %d, wms3 = %d, wms4 = %d, wms5 = %d, wms6 = %d, wms7 = %d, wms8 = %d, wms9 = %d, wms10 = %d WHERE id = %d", $_POST['markername'], $_POST['basemap'], $layer, floatval(str_replace(',', '.', $_POST['lat'])), floatval(str_replace(',', '.', $_POST['lon'])), $_POST['icon'], $_POST['popuptext'], $_POST['zoom'], $_POST['openpopup'], $_POST['mapwidth'], $_POST['mapwidthunit'], $_POST['mapheight'], $_POST['panel'], $current_user->user_login, current_time('mysql',0), $_POST['controlbox'], $_POST['overlays_custom'], $_POST['overlays_custom2'], $_POST['overlays_custom3'], $_POST['overlays_custom4'], $wms_checkbox, $wms2_checkbox, $wms3_checkbox, $wms4_checkbox, $wms5_checkbox, $wms6_checkbox, $wms7_checkbox, $wms8_checkbox, $wms9_checkbox, $wms10_checkbox, $oid );
		$wpdb->query( $result );
		$wpdb->query( "OPTIMIZE TABLE $table_name_markers" );
	        echo '<p><div class="updated" style="padding:10px;">' . __('Marker has been successfully updated','lmm') . '</div>' . __('Shortcode and API URLs','lmm') . ': <input style=\'width:200px;background:#f3efef;\' type=\'text\' value=\'[' . $lmm_options[ 'shortcode' ] . ' marker="' . $_POST['id'] . '"]\'>&nbsp;&nbsp;&nbsp;&nbsp;<a href=\'' . LEAFLET_PLUGIN_URL . '/leaflet-kml.php?marker=' . $_POST['id'] . '' .'\' target=\'_blank\' style=\'text-decoration:none;\'>KML</a> <a href=\'http://www.mapsmarker.com/kml\' target=\'_blank\' title=\'' . esc_attr__('Click here for more information on how to use as KML in Google Earth or Google Maps','lmm') . '\'> <img src=\'' . LEAFLET_PLUGIN_URL . '/img/icon-question-mark.png\' width=\'12\' height=\'12\' border=\'0\'/></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href=\'' . LEAFLET_PLUGIN_URL . '/leaflet-geojson.php?marker=' . $_POST['id'] . '&callback=jsonp' .'\' target=\'_blank\' style=\'text-decoration:none;\'>GeoJSON</a> <a href=\'http://www.mapsmarker.com/geojson\' target=\'_blank\' title=\'' . esc_attr__('Click here for more information on how to integrate GeoJSON into external websites or apps','lmm') . '\'> <img src=\'' . LEAFLET_PLUGIN_URL . '/img/icon-question-mark.png\' width=\'12\' height=\'12\' border=\'0\'/></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href=\'' . LEAFLET_PLUGIN_URL . '/leaflet-wikitude.php?marker=' . $_POST['id'] . '' .'\' target=\'_blank\' style=\'text-decoration:none;\'>Wikitude</a> <a href=\'http://www.mapsmarker.com/wikitude\' target=\'_blank\' title=\'' . esc_attr__('Click here for more information on how to display in Wikitude Augmented-Reality browser','lmm') . '\'> <img src=\'' . LEAFLET_PLUGIN_URL . '/img/icon-question-mark.png\' width=\'12\' height=\'12\' border=\'0\'></a><br/><br/>' . __('Please copy the shortcode above and paste it into the post or page where you want the map to appear','lmm') . '.<br/><br/><a class=\'button-secondary\' href=\'' . WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_marker&id=' . $_POST['id'] . '\'>' . __('edit marker','lmm') . '</a>&nbsp;&nbsp;&nbsp;'. $layer_show_button . '<a class=\'button-secondary\' href=\'' . WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_markers\'>' . __('show all markers','lmm') . '</a>&nbsp;&nbsp;&nbsp;<a class=\'button-secondary\' href=\'' . WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_marker\'>' . __('add new marker','lmm') . '</a><iframe src="http://www.mapsmarker.com/counter/go.php?id=marker_edit" frameborder="0" height="0" width="0" name="counter" scrolling="no"></iframe></p>';
	    }
		else 
		{
		echo '<p><div class="error" style="padding:10px;">' . __('Error: coordinates cannot be empty!','lmm') . '</div><br/><a href="javascript:history.back();" class=\'button-secondary\' >' . __('Go back to form','lmm') . '</a></p>';
  ...
  elseif ($action == 'delete') {
    if (!empty($oid)) {
		...
		$wpdb->query( $result );
		$wpdb->query( "OPTIMIZE TABLE $table_name_markers" );
        echo '<p><div class="updated" style="padding:10px;">' . __('Marker has been successfully deleted','lmm') . '</div><a class=\'button-secondary\' href=\'' . WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_markers\'>' . __('show all markers','lmm') . '</a><br/><br/><a class=\'button-secondary\' href=\'' . WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_marker\'>' . __('add new marker','lmm') . '</a><iframe src="http://www.mapsmarker.com/counter/go.php?id=marker_delete" frameborder="0" height="0" width="0" name="counter" scrolling="no"></iframe></p>';
    }
  }
}
else {
		...
		<table class="widefat fixed">
			...
			<tr>
				...
				<td id="wmscheckboxes">
					...
					if ($panel == 1) {
						if ($mapwidthunit == 'px') {
							echo '<div id="panel" style="display:block; background: ' . addslashes($lmm_options[ 'defaults_marker_panel_background_color' ]) . '; width:' . ($mapwidth - 10).$mapwidthunit . '; padding:5px; border:1px solid #ccc;">'.PHP_EOL;
						}
						if ($mapwidthunit == '%') {
							echo '<div id="panel" style="display:block; background: ' . addslashes($lmm_options[ 'defaults_marker_panel_background_color' ]) . '; width:' . ($mapwidth - 2).$mapwidthunit . '; padding:5px; border:1px solid #ccc;">'.PHP_EOL;
						}
						echo '<span style="' . addslashes($lmm_options[ 'defaults_marker_panel_paneltext_css' ]) . ';">' . (($markername == NULL) ? __('if set, markername will be inserted here','lmm') : $markername) . '</span><span style="float:right;width:100px;text-align:right;">';
							if ( (isset($lmm_options[ 'defaults_marker_panel_kml' ] ) == TRUE ) && ( $lmm_options[ 'defaults_marker_panel_kml' ] == 1 ) ) {
							echo '<a href="' . LEAFLET_PLUGIN_URL . '/leaflet-kml.php?marker=' . $id . '" style="text-decoration:none;" title="' . __('Export as KML for Google Earth/Google Maps','lmm') . '" target="_blank"><img src="' . LEAFLET_PLUGIN_URL . '/img/icon-kml.png" width="14" height="14" alt="KML-Logo" style="background:no-repeat;margin:0px;padding:0px;" /></a>&nbsp;';
							}
							if ( (isset($lmm_options[ 'defaults_marker_panel_geojson' ] ) == TRUE ) && ( $lmm_options[ 'defaults_marker_panel_geojson' ] == 1 ) ) {
							echo '<a href="' . LEAFLET_PLUGIN_URL . '/leaflet-geojson.php?marker=' . $id . '&callback=jsonp" style="text-decoration:none;" title="' . __('Export as GeoJSON','lmm') . '" target="_blank"><img src="' . LEAFLET_PLUGIN_URL . '/img/icon-json.png" width="14" height="14" alt="GeoJSON-Logo" style="background:no-repeat;margin:0px;padding:0px;" /></a>&nbsp;';
							}
			...
			<tr>
				<td><label for="icon"><strong><?php _e('Icon', 'lmm') ?>:</strong></label>
					...
					<small>
					<?php	
					$mapicons_admin = sprintf( esc_attr__('If you want to use different icons, please visit the %1$s (offering more than 700 compatible icons) and upload the new icons to the directory %2$s/','lmm'), '<a href="http://mapicons.nicolasmollet.com" target="_blank">Map Icons Collection</a>', LEAFLET_PLUGIN_ICONS_URL); 
					$mapicons_user = sprintf( esc_attr__('If you want to use different icons, please visit the %1$s (offering more than 700 compatible icons) and ask your WordPress admin to upload the new icons to the directory %2$s/','lmm'), '<a href="http://mapicons.nicolasmollet.com" target="_blank">Map Icons Collection</a>', LEAFLET_PLUGIN_ICONS_URL); 
