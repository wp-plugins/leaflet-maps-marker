...
if (!empty($action)) {
  ...
  if ( ($lat_check != NULL) && ($lon_check != NULL) ) {
		...
		$wms10_checkbox = isset($_POST['wms10']) ? '1' : '0';
		$layername_quotes = str_replace("\"", "'", $_POST['name']);
		$multi_layer_map_edit_link = ($_POST['multi_layer_map'] == 0) ? '<a class=\'button-primary\' href=\'' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_marker&addtoLayer=' . $_POST['id'] . '&Layername=' . urlencode($_POST['name']) . '\'>' . __('add new marker to this layer','lmm') . '</a>&nbsp;&nbsp;&nbsp;' : '';
		$mlm_checked_imploded = isset($_POST['mlm-all']) ? 'all' : '';
		if ($mlm_checked_imploded != 'all') {
			$mlm_checked_temp = '';
			foreach ($layerlist as $mlmrow){
